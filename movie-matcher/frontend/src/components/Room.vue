        ...
        <div class="card">
            <div class="room-header">
                ...
                <div class="room-status">
                    <span
                        class="status-badge"
                        :class="{ active: isMatchingActive }"
                    >
                        {{ isMatchingActive ? "Matching Active" : "Waiting" }}
                    </span>
                    <span class="participants"
                        >👥 {{ participants.length }} participant(s)</span
                    >
                </div>
            </div>
            ...
            <div class="room-content">
                ...
                <div
                    v-if="!isMatchingActive && !matchingEnded && isHost"
                    class="start-section"
                >
                ...
                >
                    ...
                    <div v-if="currentMovie" class="movie-container">
                        ...
                        <div class="movie-card">
                            ...
                            <div class="movie-details">
                                ...
                                <div class="movie-meta">
                                    <span v-if="currentMovie.year" class="badge"
                                        >📅 {{ currentMovie.year }}</span
                                    >
                                    <span
                                        v-if="currentMovie.rated"
                                        class="badge"
                                        >{{ currentMovie.rated }}</span
                                    >
                                    <span
                                        v-if="currentMovie.runtime"
                                        class="badge"
                                        >⏱ {{ currentMovie.runtime }}</span
                                    >
                                    <span
                                        v-if="currentMovie.imdb_rating"
                                        class="badge"
                                        >⭐ {{ currentMovie.imdb_rating }}</span
                                    >
                                </div>
                                <div class="movie-info">
                                    <p v-if="currentMovie.genre">
                                        <strong>Genre:</strong>
                                        {{ currentMovie.genre }}
                                    </p>
                                    <p v-if="currentMovie.director">
                                        <strong>Director:</strong>
                                        {{ currentMovie.director }}
                                    </p>
                                    <p v-if="currentMovie.actors">
                                        <strong>Actors:</strong>
                                        {{ currentMovie.actors }}
                                    </p>
                                    <p v-if="currentMovie.country">
                                        <strong>Country:</strong>
                                        {{ currentMovie.country }}
                                    </p>
                                </div>
                                <p v-if="currentMovie.plot" class="movie-plot">
                                    {{ currentMovie.plot }}
                                </p>
                            </div>
                        </div>
                        ...
                        <p class="swipe-hint">
                            Keep swiping until you find 3 common favorites!
                        </p>
                    </div>
                    ...
                    <div class="likes-sidebar">
                        ...
                        <div
                            v-if="commonLikes.length > 0"
                            class="common-likes-section"
                        >
                            <h5>
                                ✨ Common Favorites ({{ commonLikes.length }})
                            </h5>
                            <div class="common-movies-grid">
                                <div
                                    v-for="movie in commonLikes"
                                    :key="movie.imdb_id"
                                    class="common-movie-mini"
                                >
                                    <img
                        ...
                        <div class="all-likes-section">
                            <div
                                v-for="participantLikes in allParticipantLikes"
                                :key="participantLikes.participant_id"
                                class="participant-likes-group"
                            >
                                <h5>
                                    ...
                                    <span class="like-count"
                                        >{{
                                            participantLikes.liked_movies.length
                                        }}
                                        likes</span
                                    >
                                </h5>
                                <div
                                    v-if="
                                        participantLikes.liked_movies.length > 0
                                    "
                                    class="mini-movies-list"
                                >
                                    <div
                                        v-for="movie in participantLikes.liked_movies"
                                        :key="movie.imdb_id"
                                        class="mini-movie"
                                    >
                                        <img
                ...
                <div v-if="matchingEnded" class="ended-section">
                    <div
                        v-if="commonLikes.length >= 3"
                        class="match-celebration"
                    >
                        <h3>🎉 Perfect Match!</h3>
                        <p class="celebration-text">
                            You all loved these {{ commonLikes.length }} movies!
                        </p>
                    </div>
                    <div
                        v-else-if="commonLikes.length > 0"
                        class="partial-match"
                    >
                        <h3>🎬 Close Match!</h3>
                        <p class="celebration-text">
                            You found {{ commonLikes.length }} movie(s) in
                            common. Maybe try a few more?
                        </p>
                    </div>
                    <div v-else class="no-match">
                        <h3>📽️ Different Tastes!</h3>
                        <p class="celebration-text">
                            No common favorites yet. See what everyone liked
                            below!
                        </p>
                    </div>
                    ...
                    <div
                        v-if="commonLikes.length > 0"
                        class="results-section common-section"
                    >
                        ...
                        <div class="results-grid">
                            <div
                                v-for="movie in commonLikes"
                                :key="movie.imdb_id"
                                class="result-movie-card common-movie"
                            >
                                <img
                                    v-if="movie.poster !== 'N/A'"
                                    :src="movie.poster"
                                    ...
                                    class="result-poster"
                                />
                                <div v-else class="result-no-poster">
                                    No Poster
                                </div>
                    ...
                    <div class="results-section individual-section">
                        ...
                        >
                            ...
                            <div
                                v-if="participantLikes.liked_movies.length > 0"
                                class="results-grid"
                            >
                                <div
                                    v-for="movie in participantLikes.liked_movies"
                                    :key="movie.imdb_id"
                                    class="result-movie-card"
                                >
                                    <img
                                        v-if="movie.poster !== 'N/A'"
                                        :src="movie.poster"
                    ...
                    <div v-if="isHost" class="restart-section">
                        <h4>🔄 Try Again?</h4>
                        <p>
                            Adjust your filters and start a new matching session
                        </p>

                        <div class="restart-form">
                            <div class="form-group">
                                <label for="restart-genre"
                                    >Genre (optional)</label
                                >
                                <select
                                    id="restart-genre"
                                    v-model="restartFilters.genre"
                                >
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    <label for="restart-year-from"
                                        >Year From</label
                                    >
                                    <input
                                        id="restart-year-from"
                                        v-model.number="
                                            restartFilters.year_from
                                        "
                                        type="number"
                                        min="1900"
                                        max="2030"
...
function connectWebSocket() {
    ...
    const wsUrl = `${protocol}//${window.location.host}/api/rooms/${props.roomId}/ws`;
...
async function restartMatching() {
    ...
    try {
        ...
        const updateResponse = await fetch(
            `/api/rooms/${props.roomId}/filters`,
            {
                method: "PUT",
                headers: {
