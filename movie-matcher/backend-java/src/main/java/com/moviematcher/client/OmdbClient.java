...
public class OmdbClient {
    ...
    private static final Logger LOG = Logger.getLogger(OmdbClient.class);
    ...
    public Uni<MoviesPage> fetchMoviesByPage(RoomFilters filters, int page) {
        ...
        return restClient.search(apiKey, searchTerm, "movie", page, year)
            .onItem().transformToUni(searchResponse -> {
                if (!"True".equals(searchResponse.response())) {
                    String error = searchResponse.error() != null ? searchResponse.error() : "Unknown error";
                    LOG.errorf("OMDB error: %s", error);
                    return Uni.createFrom().failure(new RuntimeException("OMDB error: " + error));
                }

                List<OmdbSearchResult> searchResults = searchResponse.search() != null
                    ? searchResponse.search()
                    : List.of();

                int total = searchResponse.totalResults() != null
                    ? Integer.parseInt(searchResponse.totalResults())
                    : 0;
                boolean hasMore = (page * 10) < total;

                List<String> movieIds = searchResults.stream()
                    .filter(r -> "movie".equals(r.type()))
                    .map(OmdbSearchResult::imdbId)
                    .toList();

                if (movieIds.isEmpty()) {
                    return Uni.createFrom().item(new MoviesPage(List.of(), hasMore));
                }

                // Fetch details for all movies in parallel
                List<Uni<MovieData>> detailUnis = movieIds.stream()
                    .map(this::fetchMovieDetails)
                    .toList();

                return Uni.combine().all().unis(detailUnis)
                    .combinedWith(results -> {
                        List<MovieData> movies = results.stream()
                            .filter(obj -> obj != null)
                        ...
                        LOG.infof("Fetched %d movies from page %d", movies.size(), page);
    ...
    private Uni<MovieData> fetchMovieDetails(String imdbId) {
        return restClient.getDetails(apiKey, imdbId, "short")
            .onItem().transform(detail -> {
                ...
                return new MovieData(
                    ...
                    detail.genre(),
                    "N/A".equals(detail.imdbRating()) ? "" : detail.imdbRating(),
                    detail.imdbId()
                );
            })
            .onFailure().recoverWithItem((MovieData) null);
    }
}
